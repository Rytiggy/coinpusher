module.exports = function (database) {
    ...
    router.get("/", async (request, res) => {
        try {
            const Game = database.models.Game
            const allGames = await database.read(Game)
            console.log("getAll Games", allGames)
            res.status(200)
    ...
    router.get("/users/:id", async (request, res) => {
        try {
            ...
            const usersGame = await database.getAllByKey(Game, 'playerId', userId)
            console.log("Get users games", usersGame)
            if (usersGame) {
                res.status(200)
                return res.json(usersGame);
            } else {
                res.status(200)
                return res.json([])
            }
    ...
    router.post("/", async (request, res) => {
        try {
            ...
            let { playerId, score, type } = data
            delete data.playerId;
            delete data.score;
            delete data.type;

            const createdGame = await database.write(Game, { playerId, score, type, attributes: request.body })
            res.status(200)
            res.json(createdGame.dataValues);
            // const allGames = await database.read(Game)
            // console.log("getAll Games", allGames)
            // return res.json(allGames);
        } catch (error) {
    ...
    router.patch("/:id", async (request, res) => {
        try {
            const id = request.params.id;
            const body = request.body;
            const Game = database.models.Game
            const updatedGame = await database.update(Game, 'id', id, body)
            console.log("updatedUser", updatedGame)
            if (!updatedGame) {
                res.status(404).json({ error: "Not Found", message: "Game not found" });
                return;
            }
            res.json(updatedGame.dataValues);
        } catch (error) {
